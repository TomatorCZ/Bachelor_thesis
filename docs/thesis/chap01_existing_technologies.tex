...
A client-side code needs to control the rendered page and access a web interface providing additional services, which is usually accessible via Javascript, in order to interact with a user.
...
\par
\texttt{Worker} object represents a web worker \squarecite{11}, provided by a browser, enabling to run the script in the background.
...
WASM \squarecite{9} is a new code format that can be run in current browsers. 
...
\par
Thread \squarecite{10} support is currently discussed nowadays, and it will probably be added in future browser versions.
...
We will introduce the Common Language Infrastructure (abbreviated CLI) \squarecite{18} before diving into .NET, which is an overused name for several technologies.
...
\caption{Running a Blazor WebAssembly App on a client side \squarecite{19}.}
\label{img08:wasm}
\end{figure}
\par
...
\par
In the end, we will describe page navigation \squarecite{21} and rendering and handling events.
...
Although DOM changes is performance-expensive, a diff algorithm \squarecite{20} recognizes and tries to reduce the updates in \texttt{Batch}.
The usage of \texttt{RenderTreeBuilder} is complecated, because it has to be adapted to the algorithm \squarecite{22}.
...
\par
Blazor brings a new type of library called \ac{RCL}, which differs from normal libraries by the \textit{wwwroot} folder handling.
